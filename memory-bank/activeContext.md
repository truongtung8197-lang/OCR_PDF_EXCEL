...
- **Fix JSON parsing lỗi (mới):**
  - Thêm hàm `repair_json()` để tự động sửa lỗi Gemini thiếu key "colspan"
  - Cập nhật prompt với quy tắc #8: mọi cell PHẢI có đủ "rowspan" và "colspan"
  - Test thành công với input/1.pdf → output/1.xlsx
...
- **Thêm JSON repair function** trong gemini_extractor.py để tự động sửa lỗi syntax: `{ "rowspan": 1, 1 }` → `{ "rowspan": 1, "colspan": 1 }`
- **Cập nhật prompt** với quy tắc #8: mọi cell PHẢI có đủ 2 keys "rowspan" và "colspan"

## Bước tiếp theo

1. Test kỹ hơn với nhiều file PDF thực tế (đặc biệt là file có section_header).
2. Fix bug: "KỆ TRÊN TỦ KEM" bị Gemini nhét sai vào bảng thay vì section_header riêng.
3. Cải thiện prompt để Gemini nhận diện section_header chính xác hơn.
