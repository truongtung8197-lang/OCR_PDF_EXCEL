...
- **Fix JSON parsing lỗi (gemini_extractor.py):** Thêm hàm `repair_json()` tự động sửa lỗi thiếu key "colspan" + cập nhật prompt với quy tắc #8.
...
- **Fix JSON parsing:** thêm repair_json() + cập nhật prompt quy tắc #8 (test thành công input/1.pdf → output/1.xlsx).
